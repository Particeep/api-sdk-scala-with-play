...
object FundraiseLoanClient {
  ...
  private implicit val loan_repayment_schedule_and_user_format =
...
class FundraiseLoanClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS
  ...
  def cancelRemainingPayments(id: String, timeout: Long = defaultTimeOut)(implicit
    exec:                         ExecutionContext
  ): Future[Either[ErrorResult, List[ScheduledPayment]]] = {
