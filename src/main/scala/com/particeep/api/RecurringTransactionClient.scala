...
trait RecurringTransactionCapability {
  ...
  def recurring_transaction(credentials: ApiCredential): RecurringTransactionClient = new RecurringTransactionClient(this, Some(credentials))
}
...
class RecurringTransactionClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS
  ...
  def update(transaction_id: String, recurring_transaction_edition: RecurringTransactionEdition, timeout: Long = defaultTimeOut)(implicit
                                                                                       exec: ExecutionContext
  ): Future[Either[ErrorResult, RecurringTransaction]] = {
    ws.post[RecurringTransaction](s"$endPoint/recurring/$transaction_id", timeout, Json.toJson(recurring_transaction_edition))
  }
