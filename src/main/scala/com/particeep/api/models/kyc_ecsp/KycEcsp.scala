...
/*
 * KYC ECSP stand for : Know Your Consumer, European Crowdfunding Service Providers
 * in French : PSFP, Prestataire de Services de Financement Participatif
 *
 * Here are handeled features for our consumers doing crowdfunding type of fundraises and are bounded to european rules
 */
...
sealed trait KycEcsp {
  ...
  def isSophisticated: Boolean
  def investmentAdviceThreashold: Long
...
object KycEcsp {
  final case class Natural(
      ...
      isSophisticated:            Boolean                = false,
      investmentAdviceThreashold: Long                   = 0L,
