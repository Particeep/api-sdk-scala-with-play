...
class WalletClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials
  ...
  def allRelatedTransactions(id: String, criteria: TableSearch, timeout: Long = defaultTimeOut)(implicit
    exec:                        ExecutionContext
  ): Future[Either[ErrorResult, PaginatedSequence[TransactionWallet]]] = {
    ws.get[PaginatedSequence[TransactionWallet]](
      s"$endPoint/$id/transactions",
      timeout,
