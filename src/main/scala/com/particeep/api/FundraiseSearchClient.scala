...
class FundraiseSearchClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS
  ...
  def nbProjectsByActivityDomain(categories: List[String], timeout: Long = defaultTimeOut)(implicit
    exec:                                    ExecutionContext
  ): Future[Either[ErrorResult, List[NbProjectsByCategory]]] = {
    ws.get[List[NbProjectsByCategory]](
      s"$endPoint/info/categories",
      timeout,
      List("categories" -> categories.mkString(","))
    )
  }
}
