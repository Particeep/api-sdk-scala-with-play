...
class KycClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS with WithCredentials
  ...
  ): Future[Either[ErrorResult, List[KycGroup]]] = {
    ws.put[List[KycGroup]](s"$endPoint", timeout, Json.toJson(kyc_creation))
  }

  def update(kycs_edition: KycsEdition, timeout: Long = defaultTimeOut)(implicit
    exec:                  ExecutionContext
  ): Future[Either[ErrorResult, List[KycGroup]]] = {
  ...
  def askValidation(
    ...
    owner_ip.map(ip => {
      ws.post[List[KycGroup]](
        s"$endPoint/askValidation/owner/$owner_id/$owner_type",
        timeout,
        Json.toJson(""),
        List("owner_ip" -> ip)
      )
  ...
  def byOwnerIdAndType(owner_id: String, owner_type: String, timeout: Long = defaultTimeOut)(implicit
    exec:                        ExecutionContext
  ): Future[Either[ErrorResult, List[KycGroup]]] = {
  ...
  def cancel(
    ...
    owner_ip.map(ip => {
      ws.delete[List[KycGroup]](
        s"$endPoint/cancel/owner/$owner_id/$owner_type",
        timeout,
        Json.toJson(""),
        List("owner_ip" -> ip)
      )
  ...
  def validate(kyc_ask_validation: DocumentValidation, timeout: Long = 50000)(implicit
    exec:                          ExecutionContext
  ): Future[Either[ErrorResult, KycValidation]] = {
