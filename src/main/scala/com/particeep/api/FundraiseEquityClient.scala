...
class FundraiseEquityClient(val ws: WSClient, val credentials: Option[ApiCredential] = None) extends WithWS
  ...
  def getDismembermentSharePrice(
    id:                 String,
    dismemberment_info: DismembermentInfo,
    timeout:            Long = defaultTimeOut
  )(implicit exec:      ExecutionContext): Future[Either[ErrorResult, DismemebermentAmounts]] = {
    ws.post[DismemebermentAmounts](
      s"$endPoint/fundraise/$id/estimate/dismemberment",
      timeout,
      Json.toJson(dismemberment_info)
    )
  }
